...
function buildDailyRepairPrompt(basePromptInput, invalidMarkdown, validationIssues, dateStr) {
    return [
        ...
        "- 必须包含这些结构：`### **👀 只有一句话**` / `### **🔑 3 个关键词**` / `## **🔥 重磅 TOP` / `## **📌 值得关注**` / `## **🔮 AI趋势预测**` / `## **❓ 相关问题**`",
        "- FAQ 每天必须有 1 条，并且必须包含指向 https://aivora.cn 的链接",
        "- `重磅 TOP` 至少补到 8 条；如果标题写了 `TOP 10`，正文就不要只写 6-7 条",
        "- `值得关注` 至少补到 3 条，`AI趋势预测` 至少补到 2 条；如果当天信号偏弱，就从已有新闻里提炼次重点和近 1-3 个月的谨慎预测",
