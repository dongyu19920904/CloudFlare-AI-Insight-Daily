...
function getStandaloneDailyFunSystemPrompt() {
    return [
        ...
        getDailyFunWritingRules(),
        "正文写 5-7 个完整句子，多数句子 18-28 个可见字符，超过 35 主动拆句。来源链接只包住一个简短事实短语，不写‘官方演示视频’或‘原帖’等标签。",
    ].join('\n');
}
...
function buildDailyRepairPrompt(basePromptInput, invalidMarkdown, validationIssues, dateStr) {
    ...
    return [
        ...
        "- 每条 TOP 正文写 6-8 个完整短句，保留约 120-170 字，不压缩成几句快讯；多数句子 18-28 个可见字符，超过 35 字拆句。按结论、事实、细节、限制展开，每句只推进一个信息点。每条资讯正文默认一个紧凑自然段，短句用句号衔接，不按句数插入空行，不把结论、事实、影响分别切成三段；图片、视频和真正需要的列表或表格仍独立成块。专业栏目维持原有篇幅。完整产品名写一次，后文用明确简称；不要通过删除事实来缩短。有来源支持且具体可行的动作才给建议，不强制行动结尾。删除链接标记后句子也必须自然成立，不要写“[新闻标题](URL)显示”或“[报道详情](URL)指出”",
