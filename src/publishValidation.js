...
const OPPORTUNITY_SENSITIVE_BOUNDARY_PATTERN =
  /没有取得|没有|尚无|尚未|未获|未确认|未提供|未提取到|无(?:官方|对应|可核验|明确|已确认)|无[^。；;\n]{0,20}(?:账号|订阅|套餐)[^。；;\n]{0,12}(?:变化|更新)|待核验|待核对|待确认|仍待|仍需|仍缺|还需|有待|需要核对|需要确认|需(?:逐项)?核对|需确认|先核对|先确认|应核对|应确认|必须核对|必须确认|公开或获授权|(?:授权|许可(?:证)?)(?:条件|边界)?(?:不(?:清楚|清|明|明确)|无法(?:核对|说明))|(?:授权|许可(?:证)?)[^。；;\n]{0,20}边界无法说明|不明确|未知|缺少官方|不承诺|不能承诺|不能确认|无法确认|无法核验|无法核对|不能替代(?:官网|官方|原项目)|不含[^。；;\n]{0,24}(?:授权|许可(?:证)?)|(?:不应|不可|不足以|无法|不能|不得|不要)(?:据此|由此|因此)?[^。；;\n]{0,16}(?:包装成|写成|说成|当作|视为|解读为|宣传为|等同于|意味着|证明|确认|核验)|不等于|不等同于|(?:该|此|这个|本)(?:页面|来源|链接|仓库|项目)[^。；;\n]{0,12}不证明|不启动|不进入|不新增|不包含(?:什么)?|(?:不能|不得|不要|不应)(?:写成|说成|当作|视为|解读为|宣传为|包装成)[^。；;\n]{0,40}(?:账号|套餐|订阅|价格|额度|政策|规则|授权|服务状态|功能)|(?:并非|不是|不代表|不属于|不涉及|不改变|不调整|不更新|不受|非)[^。；;\n]{0,32}(?:官方|套餐|订阅|价格|额度|政策|规则|授权|服务状态|账号)|而非(?:套餐|价格|额度|政策|账号)(?:变化|更新)?|不触发(?:账号|商品)?上新|(?:账号)?(?:限额|封禁|登录|地区|政策)[^。；;\n]{0,24}不在售后范围|(?:交付|商品|服务|页面)?(?:说明|描述|承诺)[^。；;\n]{0,20}必须与(?:官网|官方)[^。；;\n]{0,20}一致|以[^。；;\n]{0,40}(?:页面|官网|官方)[^。；;\n]{0,20}为准|建议(?:定价|报价)|先(?:定价|报价)|试(?:挂|卖)|(?:测试|验证|试探)(?:价|价格|报价|定价)|暂定(?:价|价格)|价格假设|报价假设/;

const OPPORTUNITY_EXPLICIT_PRICE_ASSERTION_PATTERN =
  /(?:价格|售价)\s*[:：]?\s*(?:¥|￥|\$)?\s*\d|(?:¥|￥|\$)\s*\d|\d+(?:\.\d+)?\s*(?:元|美元)\s*(?:\/\s*(?:月|年))?/i;

const OPPORTUNITY_EXPLICIT_LICENSE_ASSERTION_PATTERN =
  /(?:已|已经|确认|明确)?(?:获|获得|取得|拥有|具备)[^。；;\n]{0,12}(?:授权|许可(?:证)?)|(?:采用|使用|遵循|基于|属于|是|为)[^。；;\n]{0,12}(?:MIT|Apache|GPL|开源)?\s*(?:授权|许可(?:证)?|license)|(?:授权|许可(?:证)?)(?:范围|条件)?\s*(?:是|为|包括|覆盖)/i;

const OPPORTUNITY_EXPLICIT_SENSITIVE_CHANGE_PATTERN =
  /(?:价格|售价|额度|政策|规则|服务状态|功能)[^。；;\n]{0,12}(?:已|已经|正式)?(?:调整|上涨|下降|变化|更新|生效|开放|上线|下线|停用|退役|确认|支持|允许|禁止)/i;

const OPPORTUNITY_HEDGED_SENSITIVE_CHANGE_PATTERN =
  /(?:价格|售价|额度|政策|规则|服务状态|功能)[^。；;\n]{0,8}(?:可能|或许|疑似|或将|尚待确认)(?:会|已|已经)?(?:调整|上涨|下降|变化|更新|生效|开放|上线|下线|停用|退役|支持|允许|禁止)/i;

function isBoundedOpportunitySensitiveClause(clause = "") {
  ...
  if (
    OPPORTUNITY_EXPLICIT_SENSITIVE_CHANGE_PATTERN.test(assertionText) &&
    !OPPORTUNITY_HEDGED_SENSITIVE_CHANGE_PATTERN.test(assertionText)
  ) {
    return false;
  }
  return OPPORTUNITY_SENSITIVE_BOUNDARY_PATTERN.test(clause);
}
...
function collectOpportunitySourcePolicyIssues(
  ...
  for (const block of opportunityBlocks) {
    ...
    const unsupportedCriticalFactClause = criticalFactClauses.find(
      (clause) => !isBoundedOpportunitySensitiveClause(clause)
    );
    if (
...
function collectAccountOpportunitySourceIssues(
  ...
  for (const block of actionBlocks) {
    ...
    const accountActionBoundaryPattern =
      /(?:(?:今天最小动作|今天确认|今天修改|今天记录)[^。；;\n]{0,80}(?:核对|核实|查清|列出|注明|写清|补充|补一条|修改|检查|区分|验证|确认)|(?:可以|建议)?先[^。；;\n]{0,28}(?:写|制作|补充|核对|核实|列出|说明|说清楚|注明|标明|检查|区分|验证|确认))/;
    const unsupportedCriticalFactClause = criticalFactClauses.find(
      (clause) =>
        !OPPORTUNITY_SENSITIVE_BOUNDARY_PATTERN.test(clause) &&
