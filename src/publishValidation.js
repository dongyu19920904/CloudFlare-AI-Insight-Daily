...
const OPPORTUNITY_SENSITIVE_BOUNDARY_PATTERN =
  /没有取得|没有|尚无|尚未|未获|未确认|未提供|未提取到|无(?:官方|对应|可核验|明确|已确认)|待核验|待核对|待确认|仍待|仍需|仍缺|还需|有待|需要核对|需要确认|需核对|需确认|先核对|先确认|应核对|应确认|必须核对|必须确认|不明确|未知|缺少官方|不承诺|不能确认|无法确认|无法核验|不能证明|不启动|不进入|不新增|不包含(?:什么)?|(?:并非|不是|不代表|不属于|不涉及|不改变|不调整|不更新|不受|非)[^。；;\n]{0,32}(?:官方|套餐|订阅|价格|额度|政策|规则|授权|服务状态|账号)|而非(?:套餐|价格|额度|政策|账号)(?:变化|更新)?|不触发(?:账号|商品)?上新|(?:账号)?(?:限额|封禁|登录|地区|政策)[^。；;\n]{0,24}不在售后范围|建议(?:定价|报价)|先(?:定价|报价)|试(?:挂|卖)|(?:测试|验证|试探)(?:价|价格|报价|定价)|暂定(?:价|价格)|价格假设|报价假设/;
