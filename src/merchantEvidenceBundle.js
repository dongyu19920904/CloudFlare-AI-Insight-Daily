...
export async function loadMerchantOfficialEvidence({ fetchImpl = fetch, now = new Date(), sources = MERCHANT_OFFICIAL_SOURCES } = {}) {
  return Promise.all(sources.slice(0, 4).map(async (source) => {
    try {
...
export async function buildMerchantEvidenceBundle({ dateStr, snapshot, official = [], memory = [] }) {
  ...
  return { version: EDITORIAL_VERSION, date: dateStr, evidenceKey, evidence, newEvidenceIds,
    ...
    products: products.slice(0, 12).map((p) => ({ slug: p.slug, name: p.name, platform: p.categoryId, url: p.productUrl, calculatorUrl: `https://supply.aivora.cn/profit-calculator?product=${encodeURIComponent(p.name)}`, availableOfferCount: p.availableOfferCount })),
