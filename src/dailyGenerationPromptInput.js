...
export function getDailyReadabilityRules() {
  return [
    ...
    "保留事实密度：素材充足的 TOP 正文通常写约 120–160 个可见字符，在同一自然段用 3–5 个短句交代事件、具体做法或条件、可核实结果及必要边界，至少保留两项互不重复的有用事实。后续栏目有足够素材时也写清变化和至少一项具体细节，通常约 70–110 字。这些是编辑参照，不逐条卡字数。素材只有一项可靠事实时可以短，优先换信息更完整的合格候选；不能用背景口号、重复标题或无来源推断凑长。",
