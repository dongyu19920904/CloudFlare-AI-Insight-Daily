...
test('Telegram relay link is not labeled as an organization technical statement', () => {
  const url = 'https://t.me/aigc1024/24641';
  const source = { title: 'Claude 文本水印说明', source: 'AI探索指南 - Telegram Channel', url };
  const block = `### 6. Claude 文本水印采用 SynthID-Text\n\n频道转述称，[Anthropic 技术说明](${url})介绍了水印机制。`;
  const result = quarantineDailySourceConflicts(block, [source]);
  assert.match(result.markdown, /\[频道转述内容\]\(https:\/\/t\.me\/aigc1024\/24641\)/);
  assert.doesNotMatch(result.markdown, /\[Anthropic 技术说明\]/);
  assert.equal(result.removedCount, 0);
  assert.equal(quarantineDailySourceConflicts(result.markdown, [source]).markdown, result.markdown);
