...
- 四种桌面/手机与日间/夜间浏览器验收没有正文溢出；逐张滚动加载时发现一张 `128×128` 图标源无法显示。将 URL 中明确标记为 96/128 方形缩略图的媒体归入现有低价值图片过滤，不替换原图、不改变正文事实；对应回归测试覆盖生产 URL 形态。
- 最终自动稿的 FAQ 重复出现“可直接在[报道]。”这一不完整句式。排版清理仅在“可直接在 + Markdown 链接 + 句末标点”没有后续谓语时改为“可查看链接了解详情”；正常的“可直接在链接调用接口”保持不变，不增加模型调用或 repair 阶段。
