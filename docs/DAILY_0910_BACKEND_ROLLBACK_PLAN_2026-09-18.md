...
- 当前全量 Node 测试 379/379 通过，Worker 配置等价检查 77/77 通过。
...
## 生产自动稿复核与边界修正（2026-09-19）

- 回退版 Worker 已自动生成并发布 9 月 19 日日报；发布校验硬问题为 0，证明日常自动链路已经恢复，不依赖手工改稿。
- 生产稿复核发现 `aigc**1024 分**享`：高亮后处理把账号末尾数字、单位“分”和动词“分享”错误拼接。根因是数字候选把单字“分”当作通用单位，且低价值高亮清理没有覆盖历史错误标记。
- 最小修正：数字候选保留“分钟”等完整单位，移除单字“分”；清理独立的“数字+分”和“提高时/降低后”等半截词；候选不得以“时、后、前、中”结尾。没有改提示词、素材选择、模型调用、重试次数或发布条件。
- 新增真实形态回归测试，并用线上 9 月 19 日完整 Markdown 回放：错误高亮被移除，`aigc1024 分享`恢复完整，来源 URL 序列不变。全量 Node 测试 380/380、Worker 配置等价检查 77/77 通过。
